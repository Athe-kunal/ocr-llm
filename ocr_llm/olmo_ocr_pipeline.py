...
def _get_pool(host: str, port: int, use_ssl: bool) -> asyncio.Queue:
    key = (host, port, use_ssl)
    if key not in _connection_pool:
        _connection_pool[key] = asyncio.Queue(maxsize=_POOL_MAX_IDLE)
    return _connection_pool[key]


async def _open_connection(host: str, port: int, use_ssl: bool):
    if use_ssl:
...
async def apost(url, json_data, api_key=None):
    ...
    pool = _get_pool(host, port, use_ssl)
    ...
    try:
        ...
        if api_key:
            request_headers.append(f"Authorization: Bearer {api_key}")
        ...
        # Read response body.
        server_closing = resp_headers.get("connection", "").lower() == "close"
        if "content-length" in resp_headers:
            ...
            while True:
                ...
                if chunk_size == 0:
                    await reader.readline()  # final CRLF
                    break
        ...
        # Return connection to pool if the server is keeping it alive.
        if not server_closing and not writer.is_closing():
            try:
                pool.put_nowait((reader, writer))
                writer = None  # pool now owns it; skip close in finally
            except asyncio.QueueFull:
                pass  # pool full — falls through to close in finally
    ...
    try:
        ...
        async with asyncio.TaskGroup() as tg:
            for page_num in range(1, num_pages + 1):
                task = tg.create_task(_process_page_bounded(page_num))
